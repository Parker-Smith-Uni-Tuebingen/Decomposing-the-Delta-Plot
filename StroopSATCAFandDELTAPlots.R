#Alt Analysis Template 5.0 prototype Stroop
...
setwd("~/Phase II Data Collection/Phase II Stroop Experiment Data Set 1")
...
for (i in 1:length(totaldat)){
  ...
  temp <- select(temp, type, Subject, condition, kr_trial_l.rt, kr_trial_l.corr, InstrCond)
  ...
  reA.ply <- transmute(reA.plyr,
                       ...
                       R= (Incongruent + Congruent)/2-Neutral,
                       R1= (Neutral-Congruent)/(Incongruent -Neutral))
...
dfc <- df %>% filter(df$condition == "Congruent")
dfn <- df %>% filter(df$condition == "Neutral")
dfi <- df %>% filter(df$condition == "Incongruent")
...
dfNCSA <- mkt1(out2A - out1A)
dfINSA <- mkt1(out3A - out2A)
dfDPSA <-  mkt1(out3A - out1A)
dfNCSA$q <- NCmean
dfINSA$q <- INmean
dfDPSA$q <- DPmean
...
fiStA <- ggplot(dfNCSA, aes(x = q, y = obs)) + 
  ...
  geom_line(aes(y= dfINSA$obs, color = 'Inhibition') )+
  geom_point(aes(y= dfINSA$obs))+
  geom_line(aes(y= dfDPSA$obs, color = 'Standard'), linetype="longdash" )+
  geom_point(aes(y= dfDPSA$obs))+
  ...
  labs(title = "Stroop Task - Accuracy", x = "t (s)", y = "\u0394 RT (s)")
...
colors <-c("Facilitation" = "Blue", "Inhibition" = "Red", "Standard" = "Black")
fiStB <- ggplot(dfNCB, aes(x = q, y = obs)) + 
  geom_abline(intercept = 0, slope = 0, "colour"="grey40", linetype="dashed") +
  geom_line(aes(y = obs,color = 'Facilitation'),  size = 0.5, ) +
  geom_point(size = 2) +
  # scale_colour_manual(values=c("grey40", "orange1")) +
  ...
  scale_color_manual(values = colors)+
  labs(title = "Stroop Task - Balanced", x = "t (s)", y = "")
...
dfdisSt<- data.frame(out1S, out2S, out3S)


colors <-c("Facilitation" = "Blue", "Inhibition" = "Red", "Standard" = "Black")
fiStS <- ggplot(dfNCS, aes(x = q, y = obs)) + 
  geom_abline(intercept = 0, slope = 0, "colour"="grey40", linetype="dashed") +
  geom_line(aes(y = obs,color = 'Facilitation'),  size = 0.5, ) +
  geom_point(size = 2) +
  # scale_colour_manual(values=c("grey40", "orange1")) +
  ...
  # theme(legend.position = c(.25, .8))+
  scale_color_manual(values = colors)+
  labs(title = "Stroop Task - Speed", x = "t (s)", y = "")
...
testSub1C <- filter(testSub1, testSub1$condition == "Congruent")
testSub1I <- filter(testSub1, testSub1$condition == "Incongruent")
testSub1N <- filter(testSub1, testSub1$condition == "Neutral")
...
CAFStA <- ggplot(dfa, aes(x = xl, y = stoFaA, group = 1)) + 
  geom_abline(intercept = 0, slope = 0, "colour"="grey40", linetype="dashed") +
...
CAFStB <- ggplot(dfa, aes(x = xl, y = stoFaB, group = 1)) + 
  geom_abline(intercept = 0, slope = 0, "colour"="grey40", linetype="dashed") +
...
CAFStS <- ggplot(dfa, aes(x = xl, y = stoFaS, group = 1)) + 
  geom_abline(intercept = 0, slope = 0, "colour"="grey40", linetype="dashed") +
...
grid.arrange(fiStA, fiStB, fiStS, CAFStA, CAFStB, CAFStS, ncol=3, nrow =2)
grid.arrange(fiStA, fiStB, fiStS, CAFStA, CAFStB, CAFStS, ncol=3, nrow =2)
